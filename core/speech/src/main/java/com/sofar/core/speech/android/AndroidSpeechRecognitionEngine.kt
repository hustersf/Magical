...
internal class AndroidSpeechRecognitionEngine(
  ...
  override fun start(config: SpeechRecognitionConfig): Flow<SpeechRecognitionEvent> = flow {
    ...
    startOnce(startConfig).collect { event ->
      if (event is SpeechRecognitionEvent.Error && shouldFallbackToDefaultRecognitionClient(startConfig, event.error.code)) {
        rememberUnsupportedOfflineLanguage(startConfig.languageCode)
        Log.w(TAG, "offline recognition does not support language=${startConfig.languageCode}, fallback to default recognizer")
  ...
  private fun startOnce(config: SpeechRecognitionConfig): Flow<SpeechRecognitionEvent> = callbackFlow {
    Log.d(TAG, "startOnce(config=$config)")
    if (!SpeechRecognizer.isRecognitionAvailable(appContext)) {
      trySend(SpeechRecognitionEvent.Error(AndroidSpeechErrorMapper.map(SpeechRecognizer.ERROR_CLIENT)))
      close()
      return@callbackFlow
    }
    ...
    val listener = object : RecognitionListener {
      ...
      override fun onRmsChanged(rmsdB: Float) {
        trySend(SpeechRecognitionEvent.Volume(rmsdB))
      }
      ...
      override fun onError(error: Int) {
        ...
        trySend(SpeechRecognitionEvent.Error(AndroidSpeechErrorMapper.map(error)))
        close()
      }
      ...
      override fun onResults(results: Bundle?) {
        ...
        if (!matches.isNullOrEmpty()) {
          trySend(SpeechRecognitionEvent.Text(matches[0], isFinal = true))
        }
        close()
      }
      ...
      override fun onPartialResults(partialResults: Bundle?) {
        ...
        if (!matches.isNullOrEmpty()) {
          trySend(SpeechRecognitionEvent.Text(matches[0], isFinal = false))
        }
      }
  ...
  private fun shouldFallbackToDefaultRecognitionClient(config: SpeechRecognitionConfig, error: Int): Boolean {
  ...
  private fun SpeechRecognitionConfig.withOfflineFallbackIfUnsupported(): SpeechRecognitionConfig {
