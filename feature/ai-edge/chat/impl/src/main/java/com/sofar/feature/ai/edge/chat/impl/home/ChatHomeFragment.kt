...
class ChatHomeFragment : Fragment() {
  ...
  private fun showSessionLongClickMenu(session: SessionEntity) {
    ...
    val menuItems = if (isPinned) {
      arrayOf(getString(chatR.string.feature_chat_action_clear_history)) // "清空对话记录"
    } else {
      arrayOf(
        getString(chatR.string.feature_chat_action_clear_history), // 索引 0
        getString(chatR.string.feature_chat_action_delete)        // 索引 1: "删除对话"
      )
    }

    // 2. 展开美观的 Material3 卡片级弹窗
    MaterialAlertDialogBuilder(context)
      .setTitle(session.title.ifEmpty { getString(chatR.string.feature_chat_title_default) })
